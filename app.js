...
function renderView() {
  ...
  else if (state.view === "daily") c.innerHTML = dailyList();
  else if (state.view === "cases") c.innerHTML = caseRegister();
...
function caseRegister() {
  const list = filteredCases();
  const pageCount = Math.max(1, Math.ceil(state.totalCases / state.pageSize));
  return `<div class="view-head"><div><button class="btn btn-ghost back-button" id="backBtn">‹ Back</button><span class="eyebrow">Case Register</span><h2>All Cases</h2><p>${state.totalCases} total cases</p></div></div><div class="toolbar"><input id="search" placeholder="⌕ Search case number, title or stage..." value="${esc(state.query)}"></div><div class="filter-row"><button class="chip ${!state.stage ? "active" : ""}" data-filter="">All stages</button>${CASE_STAGES.map((s) => `<button class="chip ${state.stage === s ? "active" : ""}" data-filter="${esc(s)}">${esc(s)}</button>`).join("")}</div>${
    list.length
      ? CASE_STAGES.map((stage) => {
          const items = list.filter((x) => x.stage === stage);
          return items.length
  ...
  }<div class="pagination"><button class="btn btn-secondary" id="prevPage" ${state.page <= 1 ? "disabled" : ""}>Previous</button><span>Page ${state.page} of ${pageCount} · ${state.totalCases} total</span><button class="btn btn-secondary" id="nextPage" ${state.page >= pageCount ? "disabled" : ""}>Next</button></div>`;
}
function caseCard(x) {
  const canUpdate =
...
function bindView() {
  if (["daily", "cases", "reports"].includes(state.view) && !$("backBtn")) {
    const back = document.createElement("button");
    back.id = "backBtn";
    back.className = "btn btn-ghost back-button";
